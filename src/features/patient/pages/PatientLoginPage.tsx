...
export function PatientLoginPage() {
  ...
  const login = () => {
    localStorage.setItem("auth_flow", "PATIENT");
    // AuthInitializer usa esta ruta al terminar de crear/restaurar la sesión.
    localStorage.setItem("post_login_path", "/patient/dashboard");
    return loginWithRedirect({ authorizationParams: { redirect_uri: window.location.origin, audience: import.meta.env.VITE_AUTH0_AUDIENCE, scope: "openid profile email", prompt: "login" }, appState: { returnTo: "/patient/dashboard" } });
  };
