...
export function PatientProfilePage() {
  ...
  const save = async () => {
    try {
      await patientPortalApi.updateProfile(await form.validateFields());
  ...
  const upload = async (file: File) => {
    try {
      await patientPortalApi.uploadAvatar(file);
  ...
  return (
    <Card loading={profile.isLoading} style={{ maxWidth: 700 }}>
      <Title level={2}>Mi perfil</Title>
      <Text type="secondary">
        Completa tus datos para que las clínicas te identifiquen correctamente.
      </Text>

      <div style={{ margin: "24px 0", display: "flex", alignItems: "center", gap: 16 }}>
        <Avatar size={76} src={profile.data?.avatarUrl} icon={<UserOutlined />} />
        <Upload showUploadList={false} beforeUpload={upload} accept="image/*">
          <Button icon={<InboxOutlined />}>Arrastrar o elegir foto</Button>
        </Upload>
      </div>
      ...
      <Form form={form} layout="vertical">
        ...
        <Form.Item
          ...
          rules={[{ pattern: /^[0-9+()\s-]*$/, message: "Ingresa un número de teléfono válido" }]}
        >
          <Input inputMode="tel" maxLength={50} placeholder="Ej. +51 999 999 999" />
        </Form.Item>
        <Button type="primary" onClick={save}>Guardar cambios</Button>
        <Button danger style={{ marginLeft: 12 }} onClick={signOut}>Cerrar sesión</Button>
      </Form>
    </Card>
  );
}
